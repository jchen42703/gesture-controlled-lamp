...
def generate_model(opt):
    ...
    if not opt.no_cuda:
        ...
        if opt.pretrain_path:
            ...
            pretrain = torch.load(
                opt.pretrain_path, map_location=torch.device('cpu'))
            ...
            if opt.model in ['mobilenet', 'mobilenetv2', 'shufflenet', 'shufflenetv2']:
                ...
                model.module.classifier = nn.Sequential(
                    nn.Dropout(p=0.5),
                    nn.Conv3d(
                        model.module.classifier[1].in_channels, opt.n_finetune_classes, kernel_size=1),
                    nn.ReLU(inplace=True),
                    nn.AvgPool3d((1, 4, 4), stride=1))
                model.module.classifier = model.module.classifier.cuda()
            else:
                model.module.fc = nn.Linear(
                    model.module.fc.in_features, opt.n_finetune_classes)
        ...
        if opt.pretrain_path:
            ...
            assert opt.arch == pretrain['arch']
            new_state_dict = {}
            for key, val in pretrain["state_dict"].items():
            ...
            if opt.finetune:
                if opt.model in ['mobilenet', 'mobilenetv2', 'shufflenet', 'shufflenetv2']:
                    model.module.classifier = nn.Sequential(
                        nn.Dropout(0.9),
                        nn.Linear(
                            model.module.classifier[1].in_features, opt.n_finetune_classes)
                    )
                elif opt.model == 'squeezenet':
                    model.module.classifier = nn.Sequential(
                        nn.Dropout(p=0.5),
